...
dataset = 'ultrachat' # NOTE: CHECK THIS BEFORE RUNNING SCRIPT
...
if dataset == 'edufineweb':
    ...
    eot = enc.encode_single_token('<|endoftext|>') # end of text token
    im_start = enc.encode_single_token('````') # uncommon token I chose to signal the start of assistant response
    im_end = enc.encode_single_token('?????-?????-') # uncommon token I chose to signal the end of assistant response
...
def tokenize(doc): # tokenizes a single document / shakepeare line and returns a numpy array of uint16 tokens
    tokens = [eot] # the special eot token delimits all documents / shakespeare lines

    if dataset == 'ultrachat': # conversation parsing
        for message in doc['messages']:
            if message['role'] == 'assistant': tokens.append(im_start)
            tokens.extend(enc.encode_ordinary(message['content']))
            if message['role'] == 'assistant': tokens.append(im_end)
    else:
...
if __name__ == '__main__': # prevents windows from creating infinite subroutines
