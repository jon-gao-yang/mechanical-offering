...
Notes:
- just use pytorch (less online support, "Run and Debug" mode in VScode causes an error at the end of programs that use tinygrad, tinygrad optimizers use Tensor.assign(self, x) which can only assign floats to x for some reason, so all params need to be float)
